...
> **Note**: The browser can only understand one page size for your document. If you need to create a document with different page sizes, you will need to create two separate HTML files and generate two PDFs.
...
/* Top and bottom margins are 3in, left and right margins are 2in */
@page {
  margin: 3in 4in;
}
...
You can use `:left` and `:right` pseudo-selectors on `@page` rules and style your pages differently.

Let's look at an example with different margins: outside margins are bigger than inside ones.
